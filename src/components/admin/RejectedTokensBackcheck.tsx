...
import { Loader2, CheckCircle, XCircle, RefreshCw, AlertTriangle, TrendingUp, Award, BarChart3 } from 'lucide-react';
...
export function RejectedTokensBackcheck() {
  ...
  const [stats, setStats] = useState({ total: 0, falsePositives: 0, graduated: 0, avgAth: 0 });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .select('id, token_mint, token_symbol, token_name, image_url, rejection_reason, rejected_at, ath_bonding_curve_pct, current_price_usd, current_market_cap_usd, peak_market_cap_usd, is_graduated, was_false_positive, false_positive_score, current_holders, checked_at');

      if (reasonFilter !== 'all') {
        query = query.eq('rejection_reason', reasonFilter);
      }
      if (falsePositiveOnly) {
        query = query.eq('was_false_positive', true);
      }
      if (graduatedOnly) {
        query = query.eq('is_graduated', true);
      }

      if (sortBy === 'false_positive_score') {
        query = query.order('false_positive_score', { ascending: false });
      } else if (sortBy === 'ath_bonding_curve_pct') {
        query = query.order('ath_bonding_curve_pct', { ascending: false });
      } else {
  ...
  }, [reasonFilter, falsePositiveOnly, graduatedOnly, sortBy]);

  const fetchStats = useCallback(async () => {
    try {
      const { count: total } = await supabase.from('pumpfun_rejected_backcheck').select('*', { count: 'exact', head: true });
      const { count: fps } = await supabase.from('pumpfun_rejected_backcheck').select('*', { count: 'exact', head: true }).eq('was_false_positive', true);
      const { count: grads } = await supabase.from('pumpfun_rejected_backcheck').select('*', { count: 'exact', head: true }).eq('is_graduated', true);
      const { data: avgData } = await supabase.from('pumpfun_rejected_backcheck').select('ath_bonding_curve_pct').gt('ath_bonding_curve_pct', 0).limit(1000);
      const avgAth = avgData && avgData.length > 0
        ? Math.round(avgData.reduce((s, r) => s + (r.ath_bonding_curve_pct || 0), 0) / avgData.length)
        : 0;
      setStats({ total: total || 0, falsePositives: fps || 0, graduated: grads || 0, avgAth });
    } catch {}
  }, []);
    ...
    <div className="space-y-4">
      ...
      <Card><CardContent className="p-3 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Label htmlFor="fp-only" className="text-xs">False Positives Only</Label>
        </div>
      ...
      {loading ? (
        ...
        <Table>
          ...
          <TableBody>
            {tokens.length === 0 ? (
              <TableRow><TableCell compact colSpan={8} className="text-center text-muted-foreground py-8">
