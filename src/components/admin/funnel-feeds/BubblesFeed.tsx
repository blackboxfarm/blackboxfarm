...
export function BubblesFeed() {
  const [entries, setEntries] = useState<BubbleEntry[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchEntries = async () => {
    setLoading(true);
    // Bubbles = tokens submitted via bubblemap queries or /holders command
    const { data } = await supabase
      .from('holders_intel_post_queue')
      .select('id, token_mint, symbol, name, status, trigger_source, trigger_comment, scheduled_at, posted_at')
      .or('trigger_source.ilike.%bubble%,trigger_source.ilike.%holder%,trigger_source.eq.public_query,trigger_source.eq.subscriber_query')
      .order('scheduled_at', { ascending: false })
      .limit(200);

    setEntries((data as BubbleEntry[]) || []);
    setLoading(false);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {entries.length} tokens from Bubblemap queries & /holders submissions
        </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-muted-foreground">No Bubblemap/holders query submissions yet.</div>
      ) : (
