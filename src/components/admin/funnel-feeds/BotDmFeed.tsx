...
export function BotDmFeed() {
  ...
  const fetchEntries = async () => {
    setLoading(true);
    // Bot DM = tokens submitted via Telegram bot DM or channel bot commands
    const { data } = await supabase
      .from('holders_intel_post_queue')
      .select('id, token_mint, symbol, name, status, trigger_source, trigger_comment, scheduled_at, posted_at')
      .or('trigger_source.ilike.%bot%,trigger_source.ilike.%telegram%,trigger_source.eq.bot_dm,trigger_source.eq.channel_bot')
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {entries.length} tokens from Telegram Bot DM & channel submissions
        </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-muted-foreground">No Bot DM/channel submissions yet.</div>
      ) : (
