...
type OAuthProvider = 'google' | 'x' | 'discord' | 'github';
...
export const OAuthButtons = ({ disabled = false }: OAuthButtonsProps) => {
  ...
  const handleOAuthSignIn = async (provider: OAuthProvider) => {
    ...
    try {
      ...
      if (error) {
        toast({
          title: `${getProviderName(provider)} Sign In Failed`,
  ...
  return (
    <div className="grid grid-cols-5 gap-2">
      ...
      <Button
        ...
        variant="outline"
        className="w-full"
        onClick={() => window.open('https://t.me/holdersintel_bot?start=signup', '_blank')}
        disabled={disabled}
