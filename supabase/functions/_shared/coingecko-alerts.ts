...
import { CoinGeckoErrorInfo } from "./coingecko-error-handler.ts";
...
function formatTelegramMessage(errorInfo: CoinGeckoErrorInfo, context: string): string {
  ...
  const emoji = severityEmoji[errorInfo.severity] || '⚠️';
...
<body>
  <div class="container">
    <div class="header">
      <h1 class="title">⚠️ CoinGecko API ${errorInfo.errorCode}</h1>
    </div>
...
export async function sendCoinGeckoAlert(
  ...
  try {
    ...
    result.logged = true;
    console.log('[CoinGecko Alert] ✓ Logged to database');
  } catch (e) {
  ...
  try {
    ...
    const { error: emailError } = await supabase.functions.invoke('send-email-notification', {
      body: {
        ...
        subject: `⚠️ CoinGecko ${errorInfo.errorCode} - ${errorInfo.severity.toUpperCase()}`,
        html: emailHtml,
      },
    });
    ...
    if (emailError) {
      ...
      console.log('[CoinGecko Alert] ✓ Email sent');
    }
  } catch (e) {
