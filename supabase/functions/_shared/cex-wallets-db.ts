...
let _cache = new Map<string, string>(); // wallet -> cex_name
  ...
  _loading = (async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('wallet_address, cex_name')
        .eq('chain', 'solana');
      if (error) {
      ...
      for (const r of data || []) {
        if (r.wallet_address && r.cex_name) next.set(r.wallet_address, r.cex_name);
      }
      _cache = next;
...
export function getCexNameCached(wallet: string): string | null {
  return getCexNameFile(wallet) ?? _cache.get(wallet) ?? null;
}
...
export async function recordCexWallet(opts: {
  ...
  const { wallet, cexName, cexLabel, source, verified } = opts;
  if (!wallet || !cexName) return;
  // Cache immediately
  _cache.set(wallet, cexName);
  try {
    const supabase = getClient();
    ...
    if (error) {
      ...
      console.log(`[cex-wallets-db] +1 entry: ${wallet.slice(0, 8)}... → ${cexName} (via ${source})`);
    }
  } catch (e) {
