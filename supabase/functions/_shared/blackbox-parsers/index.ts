import { BotParser, NormalizedBotFields, ParseContext } from './types.ts';
...
export function parseReply(username: string | null, text: string, ctx?: ParseContext): {
  parser: string;
  ...
  for (const p of REGISTRY) {
    if (p.matches(u)) {
      return { parser: p.displayName, fields: p.parse(text, ctx) };
    }
  }
