  ...
  for (const w of WALLETS) {
    ...
    while (pages < 5) {
      ...
      try {
        ...
        if (!res.ok) { out.push({ wallet: w, error: `helius ${res.status}` }); break; }
        txs = await res.json();
      } catch (e) { out.push({ wallet: w, error: String(e) }); break; }
      if (!Array.isArray(txs) || txs.length === 0) break;
      for (const tx of txs) {
        const transfers = tx.tokenTransfers || [];
        for (const t of transfers) {
          if (MINTS.has(t.mint) && t.toUserAccount === w) {
            // buy = received NEEGY. compute SOL spent from nativeTransfers or accountData
            let solIn = 0;
    ...
    out.push({ wallet: w, buy_count: buys.length, first: buys[0] || null, all: buys });
  }
