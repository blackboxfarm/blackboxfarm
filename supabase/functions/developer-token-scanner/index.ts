...
import { requireHeliusApiKey, heliusRestFetch } from '../_shared/helius-client.ts';
...
Deno.serve(withRunLog('developer-token-scanner', async (req) => {
      ...
      try {
        ...
        for (const tx of mintTransactions) {
          ...
          if (initMintInst?.parsed?.info?.mint) {
            ...
            try {
              const metadataResponse = await heliusRestFetch(
                '/v0/token-metadata',
                {
                  method: 'POST',
                  body: { mintAccounts: [tokenMint] },
                }
              )
