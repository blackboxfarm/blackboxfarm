...
serve(withRunLog('build-token-composite', async (req) => {
  ...
  try {
    ...
    const aiResponse = await meteredAiFetch("build-token-composite", 'https://ai.gateway.lovable.dev/v1/chat/completions', {
      method: 'POST',
      headers: {
