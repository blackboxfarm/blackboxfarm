...
-- Phase 3: Add indexes for better performance
...
-- Create validation function instead of check constraint
CREATE OR REPLACE FUNCTION public.validate_command_user_consistency()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
AS $$
BEGIN
    -- Only validate if user_id is set
    IF NEW.user_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM blackbox_wallets bw
            JOIN campaign_wallets cw ON bw.id = cw.wallet_id
            JOIN blackbox_campaigns bc ON cw.campaign_id = bc.id
            WHERE bw.id = NEW.wallet_id AND bc.user_id = NEW.user_id
        ) THEN
            RAISE EXCEPTION 'Command user_id does not match campaign owner';
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS validate_command_consistency ON blackbox_command_codes;
CREATE TRIGGER validate_command_consistency
    BEFORE INSERT OR UPDATE ON blackbox_command_codes
    FOR EACH ROW
    EXECUTE FUNCTION validate_command_user_consistency();