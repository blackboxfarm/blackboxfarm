...
-- Cron job for promo poster - runs every 30 min to check if it's time (actual interval controlled by config)
SELECT cron.schedule(
  'promo-poster-check',
  '*/30 * * * *',
  $$
  SELECT net.http_post(
    url := 'https://apxauapuusmgwbbzjgfl.supabase.co/functions/v1/promo-poster',
