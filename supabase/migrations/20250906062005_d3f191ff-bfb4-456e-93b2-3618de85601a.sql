-- First, drop all policies that depend on campaign_id column
...
-- Now we can safely drop the campaign_id column
...
-- Create new RLS policy for blackbox_wallets (independent wallets)
CREATE POLICY "Users can manage their own wallets" 
ON public.blackbox_wallets 
FOR ALL 
USING (auth.uid() IS NOT NULL);

-- Create new RLS policy for blackbox_command_codes
...
-- Create new RLS policy for blackbox_transactions
