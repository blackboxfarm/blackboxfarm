...
DO $$
BEGIN
  PERFORM cron.unschedule('nolube-channel-roster-sync-15m');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'nolube-channel-roster-sync-15m',
  '*/15 * * * *',
  $$
  SELECT net.http_post(
    url := 'https://apxauapuusmgwbbzjgfl.supabase.co/functions/v1/nolube-channel-roster-sync',
    ...
    body := '{"profile_key":"no_lube"}'::jsonb
  );
  $$
);
