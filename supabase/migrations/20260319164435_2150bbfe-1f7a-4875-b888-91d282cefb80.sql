...
-- Fix: The symbol_rank dedup was incorrectly partitioning by UPPER(symbol),
-- which collapsed different tokens with the same ticker into one row.
-- Change to partition by token_mint so each unique token gets its own row.
-- The ranked_rows CTE is only used to pick the best source for each token_mint.
...
community_agg AS (
  SELECT cl.token_mint,
    ...
    array_agg(DISTINCT a) FILTER (WHERE a IS NOT NULL) AS community_admin_handles,
    array_agg(DISTINCT m) FILTER (WHERE m IS NOT NULL) AS community_mod_handles
  FROM community_links cl
  LEFT JOIN LATERAL unnest(cl.admin_usernames) a ON true
  LEFT JOIN LATERAL unnest(cl.moderator_usernames) m ON true
...
dev_rep AS (
  ...
  FROM mesh_devs md
  JOIN LATERAL unnest(md.dev_wallets) dw ON true
  LEFT JOIN dev_wallet_reputation dwr ON dwr.wallet_address = dw
...
SELECT token_mint, symbol, name, image_url, launchpad, is_graduated, graduated_at,
  ...
  mesh_x_handles, websites, funnel_sources, was_posted, ath_24h_usd, created_at, discovery_source
FROM base_rows;

CREATE UNIQUE INDEX ON master_token_directory (token_mint);
CREATE INDEX ON master_token_directory (symbol);
CREATE INDEX ON master_token_directory (discovery_source);
CREATE INDEX ON master_token_directory (created_at DESC NULLS LAST);
