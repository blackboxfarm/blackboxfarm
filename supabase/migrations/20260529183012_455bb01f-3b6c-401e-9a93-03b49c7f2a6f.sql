SELECT cron.unschedule('no-lube-milestone-sweeper-1min')
WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'no-lube-milestone-sweeper-1min');

SELECT cron.schedule(
  'no-lube-milestone-sweeper-1min',
  '* * * * *',
  $$
  SELECT net.http_post(
    url := 'https://apxauapuusmgwbbzjgfl.supabase.co/functions/v1/no-lube-milestone-sweeper',
