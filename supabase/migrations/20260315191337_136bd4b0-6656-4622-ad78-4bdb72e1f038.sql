-- Recreate view: exclude rejected/dead tokens at the output level
...
mesh_devs AS (
  ...
  WHERE rm.linked_type = 'token' AND rm.source_type = 'wallet' AND rm.relationship IN ('created', 'created_token', 'dev_wallet')
  GROUP BY rm.linked_id
),
mesh_websites AS (
  SELECT w.token_mint, array_agg(DISTINCT w.site) AS websites
  FROM (
    SELECT rm.source_id AS token_mint, rm.linked_id AS site FROM reputation_mesh rm WHERE rm.source_type = 'token' AND rm.linked_type = 'website'
    UNION ALL
    SELECT rm.linked_id, rm.source_id FROM reputation_mesh rm WHERE rm.linked_type = 'token' AND rm.source_type = 'website'
  ) w GROUP BY w.token_mint
),
mesh_x AS (
  SELECT x.token_mint, array_agg(DISTINCT x.handle) AS x_handles
  FROM (
    SELECT rm.source_id AS token_mint, rm.linked_id AS handle FROM reputation_mesh rm WHERE rm.source_type = 'token' AND rm.linked_type = 'twitter'
    UNION ALL
    SELECT rm.linked_id, rm.source_id FROM reputation_mesh rm WHERE rm.linked_type = 'token' AND rm.source_type = 'twitter'
  ) x GROUP BY x.token_mint
),
dev_rep AS (
...
kyc_data AS (
  SELECT DISTINCT ON (dp.master_wallet_address) dp.master_wallet_address,
    dp.kyc_verified, dp.kyc_source
...
-- Indexes
CREATE UNIQUE INDEX idx_mtd_token_mint ON master_token_directory (token_mint);
CREATE INDEX idx_mtd_creator_wallet ON master_token_directory (creator_wallet);
CREATE INDEX idx_mtd_symbol ON master_token_directory (symbol);