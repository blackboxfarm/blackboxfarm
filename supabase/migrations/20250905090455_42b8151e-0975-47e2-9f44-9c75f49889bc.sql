-- Fix the JSON parsing errors in the cron function
    ...
    LOOP
        ...
        IF last_buy_time IS NULL OR last_buy_time < (now() - (buy_interval || ' seconds')::interval) THEN
            ...
            BEGIN
                ...
                response_content := COALESCE(http_response.content::text, '{}');
                
                BEGIN
                    parsed_response := response_content::jsonb;
                EXCEPTION WHEN OTHERS THEN
                ...
                VALUES (
                    ...
                    jsonb_build_object(
                        ...
                        'raw_content_length', length(response_content)
                    )
                );
            EXCEPTION WHEN OTHERS THEN
                -- Log any errors
