# Archived Edge Function: liquidity-lock-checker

Stopped and deleted on 2026-07-19 to free up Supabase edge function slots and stop Helius/Solscan credit burn. Original source preserved below for later reference or restoration.

Callers left in place (they tolerate empty responses):
- src/components/LiquidityLockChecker.tsx
- supabase/functions/telegram-channel-monitor/index.ts
- supabase/functions/token-performance-analyzer/index.ts

## Original source (`supabase/functions/liquidity-lock-checker/index.ts`)
...
serve(withRunLog('liquidity-lock-checker', async (req) => {
